...
from piqa_tools import get_emb_data_path, load_embedder
from piqa_data import PIQAEmbeddingDataset, get_tokenized_samples, load_emb_data
...
def parseArguments(): 
    ...
    parser.add_argument("--cls_type", type=str, default="mlp")
    ...
    train_data_path, train_label_path = get_emb_data_path(args, split="train") 
    valid_data_path, valid_label_path = get_emb_data_path(args, split="valid")
...
def get_output_name(args):
    output_name = "probing_{}_{}_R{}_S{}_B{}_LR{}".format(
        args.embedder, args.cls_type, args.num_runs, args.train_step, args.batch_size, args.learning_rate
    )
...
def eval(model, data, valid_id, args):
    ...
    with torch.no_grad():
        for iter_id, batch in enumerate(loader):
            embeddings, labels = tuple(t.to(device) for t in batch)
            logits, _ = model(embeddings, labels, is_challenge=args.is_challenge)
            ...
            if not args.is_challenge:
                ...
                labels_onehot = torch.Tensor(embeddings.size(0), embeddings.size(1)).cpu()
                labels_onehot.zero_()
...
def main(args):
    ...
    train_samples = load_emb_data(args.train_data_path, args.train_label_path, args.input_size, num_rows=args.num_rows)
    valid_samples = load_emb_data(args.valid_data_path, args.valid_label_path, args.input_size)

    # Retrieve PIQADataset
    train_dataset = PIQAEmbeddingDataset(train_samples["embedding"], train_samples["label"])
    valid_dataset = PIQAEmbeddingDataset(valid_samples["embedding"], valid_samples["label"])
    ...
    for run_id in range(args.num_runs):
        ...
        # Create classifier
        model = MultipleChoiceProbingHead(args.input_size, args.hidden_size, cls_type=args.cls_type, hidden_dropout=args.dropout)
        model.to(device)
        ...
        while patience < args.patience:
            ...
            for iter_id in range(args.train_step):
                ...
                embeddings, labels = tuple(t.to(device) for t in batch)
                logits, loss = model(embeddings, labels, is_challenge=args.is_challenge)
                loss.backward()
                optimizer.step()
                model.zero_grad()
